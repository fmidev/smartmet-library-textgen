...
{
  theOutput << endl
            << "thePeriod: " << as_string(theWindEventPeriodDataItem.thePeriod) << endl
            << "theWindSpeedChangePeriod: "
            << as_string(theWindEventPeriodDataItem.theWindSpeedChangePeriod) << endl
...
void log_wind_event_periods(wo_story_params& storyParams)
{
  ...
  for (auto & i : windEventPeriodVector)
  {
    ...
    storyParams.theLog << i->thePeriod.localStartTime() << "..."
    ...
    {
      float maxWindBeg =
          i->thePeriodBeginDataItem.theEqualizedTopWind.value();
      float maxWindEnd = i->thePeriodEndDataItem.theEqualizedTopWind.value();
      float medianWindBeg =
          i->thePeriodBeginDataItem.theEqualizedMedianWind.value();
      float medianWindEnd =
          i->thePeriodEndDataItem.theEqualizedMedianWind.value();
             ...
             windEventId == MISSING_WIND_DIRECTION_EVENT)
    {
      WeatherResult directionBeg(
          i->thePeriodBeginDataItem.theEqualizedWindDirection);
      WeatherResult directionEnd(
          i->thePeriodEndDataItem.theEqualizedWindDirection);
    ...
    wo_story_params& storyParams, const WindEventPeriodDataItem& dataItem)
{
  ...
  std::vector<SpeedDifferencePeriod> missingSpeedDifferenceVector;
  ...
  while (timeIter <= dataItem.thePeriod.localEndTime())
  {
    ...
    if (!timeIterInside)
    {
      ...
      {
        const WindDataItemUnit& begDataItem = get_data_item(storyParams, missingStartTime);
          ...
          dataItem.thePeriod.localStartTime() < p->thePeriod.localEndTime())
      {
        ...
        if (newWindEvent != MISSING_WIND_SPEED_EVENT)
        {
          ...
          storyParams.theLog << "Missing wind speed event period " << as_string(p->thePeriod)
                             ...
                             << get_wind_event_string(p->theWindEvent) << " and "
                             << as_string(newPeriod2) << " -> "
...
float WindDataItemUnit::getWindSpeedShare(float theLowerLimit, float theUpperLimit) const
{
  ...
  {
    if (i.first >= theLowerLimit &&
        i.first < theUpperLimit)
      retval += i.second.value();
  }
