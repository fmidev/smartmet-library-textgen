                   ...
                   double theWindDirectionMinSpeed)
{
  try
  {
    theOutput << theWindDataItem.thePeriod.localStartTime() << " ... "
              ...
              << "; suunnan k-hajonta: " << fixed << setprecision(4)
              << theWindDataItem.theCorrectedWindDirection.error()
                     ...
                     const bitset<14>& theColumnSelectionBitset)
{
  ...
  {
    throw Fmi::Exception::Trace(BCP, "Operation failed")
        .addParameter("fileIdentifierString", fileIdentifierString)
...
void save_raw_data(wo_story_params& storyParams, const string& id_str = "_original")
{
  ...
  {
    throw Fmi::Exception::Trace(BCP, "Operation failed").addParameter("id_str", id_str);
  }
}
                               ...
                               WindShareFn shareOf)
{
  ...
  {
    throw Fmi::Exception::Trace(BCP, "Operation failed")
        .addParameter("startIdx", Fmi::to_string(startIdx))
        .addParameter("endIdx", Fmi::to_string(endIdx));
  }
}
}  // namespace
    ...
    for (unsigned int k = 0; k < storyParams.theWeatherAreas.size(); k++)
    {
      ...
      for (auto& i : storyParams.theWindDataVector)
      {
        ...
        csv_data << windDataItem.thePeriod.localStartTime() << ", " << fixed << setprecision(4)
                 ...
                 << windDataItem.theCorrectedWindDirection.value() << ", " << fixed
                 << setprecision(4) << windDataItem.theCorrectedWindDirection.error() << ", "
                 << fixed << setprecision(4) << windDataItem.theEqualizedWindDirection.value()
                 << ", " << fixed << setprecision(4) << windDataItem.theGustSpeed.value() << '\n';
      }

      std::string csv_filename = Settings::require_string("qdtext::outdir") + "html/" +
                                 storyParams.theArea.name() + ".csv";
...
void log_windirection_distribution(wo_story_params& storyParams)
{
  try
  {
    ...
    ss << std::setw(31) << std::setfill(' ') << std::right << "POH" << std::setw(12)
       << std::setfill(' ') << std::right << "KOI" << std::setw(12) << std::setfill(' ')
       << std::right << "ITÄ" << std::setw(12) << std::setfill(' ') << std::right << "KAA"
       << std::setw(12) << std::setfill(' ') << std::right << "ETE" << std::setw(12)
       << std::setfill(' ') << std::right << "LOU" << std::setw(12) << std::setfill(' ')
       << std::right << "LÄN" << std::setw(12) << std::setfill(' ') << std::right << "LUO"
                                 ...
                                 const WeatherArea& weatherArea)
{
  ...
  {
    throw Fmi::Exception::Trace(BCP, "Operation failed").addParameter("i", Fmi::to_string(i));
  }
}
...
void find_out_wind_speed_event_periods(wo_story_params& storyParams)
{
  try
  {
    ...
    for (WindEventPeriodDataItem* windEventPeriodDataItem :
         storyParams.theWindSpeedEventPeriodVector)
    {
    ...
    for (unsigned int j = 0; j < storyParams.theWeatherAreas.size(); j++)
    {
      ...
      while (true)
      {
        ...
        for (unsigned int i = 0; i < eqIndexVector.size() - 2; i++)
        {
          ...
          const WindDataItemUnit& dataItemIndex1 =
              (*storyParams.theWindDataVector[index1])(areaType);
          const WindDataItemUnit& dataItemIndex2 =
              (*storyParams.theWindDataVector[index2])(areaType);
          const WindDataItemUnit& dataItemIndex3 =
              (*storyParams.theWindDataVector[index3])(areaType);
...
//      others (configurable margin) the cell is tagged with that quadrant; otherwise the
...
std::vector<ConvectiveCellAnomaly> detect_convective_anomalies(const wo_story_params& storyParams)
{
  try
  {
    std::vector<ConvectiveCellAnomaly> anomalies;

    if (storyParams.theConvectiveCellMaxDuration <= 0.0 &&
        storyParams.theConvectiveCellMaxAreaFraction <= 0.0)
      return anomalies;
    ...
    while (i < n)
    {
      ...
      const bool withinDuration =
          (maxDuration <= 0.0 || static_cast<double>(durationHours) < maxDuration);
      ...
      if (!withinDuration)
      {
        storyParams.theLog << "Convective candidate run rejected: " << period.localStartTime()
                           << " - " << period.localEndTime() << " duration=" << durationHours
                           << "h >= max duration=" << maxDuration
                           << "h (treated as synoptic, not a cell).\n";
        continue;
      }
...
// Iterates every cell period and every (timestep, area) pair inside it, removing the cell.
...
// Picks the dominant quadrant for one cell period, or returns WeatherArea::Full when the
...
void read_configuration_params(wo_story_params& storyParams)
{
  try
  {
    ...
    std::string convectiveCellStyle =
        Settings::optional_string(storyParams.theVar + "::convective_cell_style", "sentence");
  ...
  try
  {
    ...
    if (populate_time_series(storyParams))
    {
      ...
      if (Settings::optional_bool("qdtext::append_graph", false))
      {
        ...
        html_string += "<h5>";
        Fmi::Date startDate(
            Fmi::Date::from_iso_string(itsPeriod.localStartTime().ToStr(kYYYYMMDD)));
