                                    ...
                                    int& theNumberDivisibleByFive)
{
  try
  {
    ...
    bool exactMatch = (theMaximum == theMinimumCalc &&
                       theMaximum == static_cast<float>(theNumberDivisibleByFive));
...
period_area_ids get_period_area_ids(forecast_period_id thePeriodId, forecast_area_id theAreaId)
{
  try
  {
    ...
    auto pick =
        [theAreaId](weather_result_id inland, weather_result_id coast, weather_result_id area)
    ...
    if (thePeriodId == DAY1_PERIOD)
    {
      ...
      ids.min_morning =
          pick(INLAND_MIN_DAY1_MORNING, COAST_MIN_DAY1_MORNING, AREA_MIN_DAY1_MORNING);
      ids.max_morning =
          pick(INLAND_MAX_DAY1_MORNING, COAST_MAX_DAY1_MORNING, AREA_MAX_DAY1_MORNING);
      ids.mean_morning =
    ...
    else  // DAY2_PERIOD
    {
      ...
      ids.min_morning =
          pick(INLAND_MIN_DAY2_MORNING, COAST_MIN_DAY2_MORNING, AREA_MIN_DAY2_MORNING);
      ids.max_morning =
          pick(INLAND_MAX_DAY2_MORNING, COAST_MAX_DAY2_MORNING, AREA_MAX_DAY2_MORNING);
      ids.mean_morning =
                 ...
                 Sentence& theTemperaturePhrase)
{
  try
  {
    ...
    if (p.theUseFrostExistsPhrase)
    {
      ...
      {
        theDayPhasePhrase
            << (p.theForecastPeriodId == NIGHT_PERIOD ? YOLLA_PHRASE : PAIVALLA_PHRASE);
      }
  ...
  try
  {
    ...
    if (p.theSeasonId == SUMMER_SEASON)
    {
      ...
      else if (p.theForecastPeriodId != NIGHT_PERIOD)
      {
        ...
        plainIsVerbUsed = handle_summer_day_phrase(
            p, useDayTemperaturePhrase, theDayPhasePhrase, dayPhaseString);
      }
    }
    else
    {
