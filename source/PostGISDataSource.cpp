		...
		if(reg)
		  {
			...
			OGRDataSource* pDS = reg->GetDriverByName("PostgreSQL")->Open(connection_ss.str().c_str(), FALSE);
		...
		while((pFeature = pLayer->GetNextFeature()) != NULL)
		  {
			...
			if(pGeometry)
			  {				
				...
				OGRwkbGeometryType geometryType = static_cast<OGRwkbGeometryType>(pGeometry->getGeometryType() & (~wkb25DBit));
