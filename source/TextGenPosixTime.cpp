...
TextGenPosixTime::TextGenPosixTime(const TextGenPosixTime& theTime) : istPosixTime(theTime.istPosixTime)
{
}

TextGenPosixTime::TextGenPosixTime(const NFmiStaticTime& theTime) : istPosixTime(date(theTime.GetYear(), 
																					  theTime.GetMonth(), 
																					  theTime.GetDay()), 
																				 time_duration(theTime.GetHour(),
																							   theTime.GetMin(),
																							   theTime.GetSec()))
{
}

TextGenPosixTime::TextGenPosixTime(short year, short month, short day) : istPosixTime(date(year, month, day), time_duration(0,0,0))
{
}

TextGenPosixTime::TextGenPosixTime(short year, short month, short day, short hour, short minute/*=0*/, short sec/*=0*/)
{
...
short TextGenPosixTime::GetYear(void) const
{
  return istPosixTime.date().year();
}

short TextGenPosixTime::GetMonth(void) const
{
  return istPosixTime.date().month();
}

short TextGenPosixTime::GetDay(void) const
{
  return istPosixTime.date().day();
}

short TextGenPosixTime::GetHour(void) const
{
  return istPosixTime.time_of_day().hours();
}

short TextGenPosixTime::GetMin(void) const
{
  return istPosixTime.time_of_day().minutes();
}

short TextGenPosixTime::GetSec(void) const
{
...
std::string TextGenPosixTime::ToStr(const unsigned long theTimeMask) const
{
  ...
  if(kMonth & theTimeMask)
	ss << std::setw(2) << setfill('0') << GetMonth();
  
  if(kDay & theTimeMask)
	ss << std::setw(2) << setfill('0') << GetDay();

  if(kHour & theTimeMask)
	ss << std::setw(2) << setfill('0') << GetHour();

  if(kMinute & theTimeMask)
	ss << std::setw(2) << setfill('0') << GetMin();

  if(kSecond & theTimeMask)
	ss << std::setw(2) << setfill('0') << GetSec();
...
short TextGenPosixTime::GetZoneDifferenceHour(const TextGenPosixTime& theTime, bool isUtc)
{
  ...
  if(dst_local_start_time > dst_local_end_time)
	{
	  if(isUtc)
		dst_on = (theTime.istPosixTime + hours(dst_offset) >= dst_local_start_time || theTime.istPosixTime + hours(dst_offset) < dst_local_end_time);	
	  else
		dst_on = (theTime.istPosixTime >= dst_local_start_time || theTime.istPosixTime < dst_local_end_time);
	}
  else
	{
	  if(isUtc)
...
short TextGenPosixTime::GetJulianDay(void) const
{
