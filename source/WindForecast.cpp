...
Paragraph WindForecast::getWindStory(const WeatherPeriod& thePeriod) const
{
  ...
  for (unsigned int i = 0; i < theParameters.theMergedWindEventPeriodVector.size(); i++)
  {
    ...
    if (theParameters.theMergedWindEventPeriodVector.size() > 1 && eventId == MISSING_WIND_EVENT)
    {
          ...
          windSpeedIntervalPeriodPreviousReported.localStartTime().GetYear() != 1971)
      {
        ...
        int windSpeedDiff = wind_speed_difference(theParameters.theSources,
                                                  theParameters.theArea,
                                                  windSpeedIntervalPeriodPreviousReported,
                                                  windSpeedPeriod,
                                                  theParameters.theWindDataVector);
        windSpeedDifferEnough = abs(windSpeedDiff) >= 3;
      ...
      {
        windSpeedDifferEnough = wind_speed_differ_enough(theParameters.theSources,
                                                         theParameters.theArea,
                                                         eventPeriod,
                                                         theParameters.theWindSpeedThreshold,
                                                         theParameters.theWindDataVector);
      }

      windSpeedDifferEnoughOnNextPeriod =
          (eventPeriodItemNext && wind_speed_differ_enough(theParameters.theSources,
                                                           theParameters.theArea,
    ...
    int windSpeedUpperLimit(
        get_maximum_wind(eventStartPeriod, theParameters.theArea, theParameters.theWindDataVector));
    ...
    switch (eventId)
    {
      ...
      case TUULI_VOIMISTUU:
      {
        ...
        if (firstSentenceInTheStory)
        {
          ...
          else
          {
            ...
            if(eventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
                  ...
                  {
                    sentence << (eventId == TUULI_HEIKKENEE ?
            POHJOISTUULI_HEIKKENEE_NOPEASTI_COMPOSITE_PHRASE :
            POHJOISTUULI_VOIMISTUU_NOPEASTI_COMPOSITE_PHRASE)
                                     << directionSentence
                                     << changeAttributeStr;
                  }
            */
          ...
          if (useAlkaaHeiketaVoimistuaPhrase)
          {
            ...
            else
            {
              if (windStrengthStartsToChange)
              {
                ...
                {
                  sentence
                      << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_ALKAA_HEIKETA_NOPEASTI_COMPOSITE_PHRASE
        ...
        if (appendDecreasingIncreasingInterval && reportWindSpeed)
        {
          sentence << decreasingIncreasingInterval(
              *eventPeriodItem, firstSentenceInTheStory, eventId);
          windSpeedIntervalPeriodPreviousReported =
      ...
      case TUULI_KAANTYY_JA_HEIKKENEE:
      {
        ...
        if (!sentence.empty() && appendDecreasingIncreasingInterval)
        {
          reportWindSpeed = (eventPeriodItem->theWindSpeedChangeEnds ||
                             (!eventPeriodItem->theWindSpeedChangeEnds && windSpeedDifferEnough &&
                              windSpeedDifferEnoughOnNextPeriod));
          ...
          if (firstSentenceInTheStory)
          {
            if (eventPeriodItem->theTransientDirectionChangeFlag)
            {
              if (eventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
              {
                if (windStrengthStartsToChange)
                  sentence
                      << VOIMAKAS_POHJOISTUULI_ALKAA_HEIKETA_JA_MUUTTUU_TILAPAISESTI_VAIHTELEVAKSI_COMPOSITE_PHRASE
                      << windDirectionSentenceWithAttribute(windDirectionIdAvg);
                else
                  sentence
                      << VOIMAKAS_POHJOISTUULI_HEIKKENEE_JA_MUUTTUU_TILAPAISESTI_VAIHTELEVAKSI_COMPOSITE_PHRASE
                      << windDirectionSentenceWithAttribute(windDirectionIdAvg);
              }
              else
              {
            ...
            else
            {
              if (eventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
              {
                if (windStrengthStartsToChange)
                  sentence
                      << VOIMAKAS_POHJOISTUULI_ALKAA_HEIKETA_JA_MUUTTUU_VAIHTELEVAKSI_COMPOSITE_PHRASE
                      << windDirectionSentenceWithAttribute(windDirectionIdAvg);
                else
                  sentence
                      << VOIMAKAS_POHJOISTUULI_HEIKKENEE_JA_MUUTTUU_VAIHTELEVAKSI_COMPOSITE_PHRASE
                      << windDirectionSentenceWithAttribute(windDirectionIdAvg);
              }
              else
              {
          ...
          else
          {
            if (eventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
            {
              if (eventPeriodItem->theTransientDirectionChangeFlag)
              {
                if (windStrengthStartsToChange)
                  sentence
                      << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_ALKAA_HEIKETA_JA_MUUTTUU_TILAPAISESTI_VAIHTELEVAKSI_COMPOSITE_PHRASE;
                else
                  sentence
                      << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_HEIKKENEE_JA_MUUTTUU_TILAPAISESTI_VAIHTELEVAKSI_COMPOSITE_PHRASE;
              }
              else
              {
                if (windStrengthStartsToChange)
                  sentence
                      << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_ALKAA_HEIKETA_JA_MUUTTUU_VAIHTELEVAKSI_COMPOSITE_PHRASE;
                else
                  sentence
                      << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_HEIKKENEE_JA_MUUTTUU_VAIHTELEVAKSI_COMPOSITE_PHRASE;
              }
            ...
            else
            {
              ...
              else
              {
                if (windStrengthStartsToChange)
                  ...
                  sentence
                      << ILTAPAIVALLA_TUULI_HEIKKENEE_JA_MUUTTUU_VAIHTELEVAKSI_COMPOSITE_PHRASE;
              }
              sentence << getTimePhrase(eventStartPeriod, !firstSentenceInTheStory);
            }
          }
...
Sentence WindForecast::windDirectionAndSpeedChangesSentence(
    const wo_story_params& theParameters,
    const WeatherPeriod& eventPeriod,
    ...
    bool& appendDecreasingIncreasingInterval) const
{
    ...
    else
    {
          ...
          else
          {
            if (currentWindEventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
            {
              if (theWindSpeedChangeStarts && !theWindSpeedChangeEnds)
              {
                sentence
                    << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_ALKAA_HEIKETA_NOPEASTI_COMPOSITE_PHRASE
                    << getTimePhrase(eventPeriod, useAlkaenPhrase) << VOIMAKAS_TUULI_PHRASE
                    << (smallChange ? EMPTY_STRING : changeAttributeStr);
                appendDecreasingIncreasingInterval = false;
              }
              else
      ...
      else
      {
        if (previousWindEventId & TUULI_VOIMISTUU)
        {
          ...
          else
          {
            if (currentWindEventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
            {
              ...
              {
                sentence
                    << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_ALKAA_HEIKETA_JA_KAANTYY_ETELAAN_COMPOSITE_PHRASE;
                appendDecreasingIncreasingInterval = false;
              }
              else
              {
                sentence
                    << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_HEIKKENEE_JA_KAANTYY_ETELAAN_COMPOSITE_PHRASE;
              }
              sentence << getTimePhrase(eventPeriod, useAlkaenPhrase)
        ...
        else
        {
          ...
          {
            sentence
                << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_KAANTYY_ETELAAN_JA_HEIKKENEE_EDELLEEN_COMPOSITE_PHRASE
                << getTimePhrase(eventPeriod, useAlkaenPhrase)
                << windDirectionSentenceWithAttribute(windDirectionIdBeg)
                << getWindDirectionTurntoString(windDirectionIdEnd);
          }
  ...
  else
  {
    ...
    WindDirectionId windDirectionIdBegPlus1(
        get_wind_direction_id_at(theParameters.theWindDataVector,
                                 theParameters.theArea,
                                 begTimePlus1,
                                 theParameters.theVar));
    WindDirectionId windDirectionIdAvgPlus1(
        get_wind_direction_id_at(theParameters.theWindDataVector,
                                 theParameters.theArea,
                                 theParameters.theSources,
    ...
    if (firstSentenceInTheStory)
    {
      if (windDirectionIdBeg == VAIHTELEVA || windDirectionIdBeg == MISSING_WIND_DIRECTION_ID)
      {
        ...
        if (windDirectionIdBegPlus1 == windDirectionIdEnd || !directionTurns)
        {
          if (theWindSpeedChangeStarts && !theWindSpeedChangeEnds)
          {
            ...
            else
            {
              if (currentWindEventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
                sentence
                    << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_ALKAA_HEIKETA_NOPEASTI_COMPOSITE_PHRASE
                    << timePhrase << windDirectionSentenceWithAttribute(windDirectionIdAvgPlus1)
                    << (smallChange ? EMPTY_STRING : changeAttributeStr);
              else
                sentence << ILTAPAIVALLA_ETELATUULI_ALKAA_HEIKETA_NOPEASTI_COMPOSITE_PHRASE
          ...
          {
            if (strengtheningWind)
              sentence << ILTAPAIVALLA_ETELATUULI_VOIMISTUU_NOPEASTI_COMPOSITE_PHRASE << timePhrase
            ...
            else
            {
              if (currentWindEventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
                ...
                sentence << ILTAPAIVALLA_ETELATUULI_HEIKKENEE_NOPEASTI_COMPOSITE_PHRASE
                         << timePhrase << windDirectionSentence(windDirectionIdAvgPlus1, true)
                         << changeAttributeStr;
            }
          }
        ...
        else
        {
          ...
          else
          {
            if (currentWindEventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
              sentence
                  << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_HEIKKENEE_NOPEASTI_JA_KAANTYY_ETELAAN_COMPOSITE_PHRASE
                  << timePhrase << windDirectionSentenceWithAttribute(windDirectionIdBegPlus1)
                  << changeAttributeStr << getWindDirectionTurntoString(windDirectionIdEnd);
            else
              sentence
      ...
      else
      {
        ...
        if (directionTurns)
        {
          WeatherPeriod windTurnignPeriod(get_wind_turning_period(
              eventPeriod, theParameters.theWindDirectionEventPeriodVector));
          ...
          if (theWindSpeedChangeStarts && !theWindSpeedChangeEnds)
          {
            ...
            sentence << timePhrase << windDirectionSentence(windDirectionIdBeg)
                     << windSpeedIntervalSentence(WeatherPeriod(eventPeriod.localStartTime(),
                                                                eventPeriod.localStartTime()),
                                                  DONT_USE_AT_ITS_STRONGEST_PHRASE)
                     << (smallChange ? EMPTY_STRING : changeAttributeStr) << timePhrase2
                     << getWindDirectionTurntoString(windDirectionIdEnd);
            windSpeedIntervalPeriodPreviousReported =
                WeatherPeriod(eventPeriod.localStartTime(), eventPeriod.localStartTime());
            appendDecreasingIncreasingInterval = false;
          ...
          else
          {
            if (currentWindEventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
              sentence
                  << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_HEIKKENEE_NOPEASTI_JA_KAANTYY_ILLALLA_ETELAAN_COMPOSITE_PHRASE
                  << timePhrase << windDirectionSentenceWithAttribute(windDirectionIdBeg)
                  << changeAttributeStr << timePhrase2
                  << getWindDirectionTurntoString(windDirectionIdEnd);
            else
              sentence
                  ...
                  << timePhrase << changeAttributeStr
                  << (strengtheningWind ? VOIMISTUVAA_WORD : HEIKKENEVAA_WORD)
                  << windDirectionSentence(windDirectionIdBeg) << timePhrase2
                  << getWindDirectionTurntoString(windDirectionIdEnd);
          }
          previousWindDirectionId = windDirectionIdEnd;
        }
        else
        {
          if (theWindSpeedChangeStarts && !theWindSpeedChangeEnds)
          {
            if (strengtheningWind)
              sentence << ILTAPAIVALLA_ETELATUULI_ALKAA_VOIMISTUA_NOPEASTI_COMPOSITE_PHRASE
                       << timePhrase << windDirectionSentence(windDirectionIdAvg)
                       << (smallChange ? EMPTY_STRING : changeAttributeStr);
            ...
            else
            {
              if (currentWindEventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
                sentence
                    << ILTAPAIVALLA_VOIMAKAS_POHJOISTUULI_ALKAA_HEIKETA_NOPEASTI_COMPOSITE_PHRASE
                    << timePhrase << windDirectionSentenceWithAttribute(windDirectionIdAvg)
                    << (smallChange ? EMPTY_STRING : changeAttributeStr);
              else
                sentence << ILTAPAIVALLA_ETELATUULI_ALKAA_HEIKETA_NOPEASTI_COMPOSITE_PHRASE
                         << timePhrase << windDirectionSentence(windDirectionIdAvg)
          ...
          else
          {
            if (currentWindEventId == TUULI_HEIKKENEE && windSpeedUpperLimit >= GALE_LOWER_LIMIT)
              ...
              sentence
                  ...
                  << timePhrase << changeAttributeStr
                  << (strengtheningWind ? VOIMISTUVAA_WORD : HEIKKENEVAA_WORD)
                  << windDirectionSentence(windDirectionIdBegPlus1) << timePhrase2
                  << getWindDirectionTurntoString(windDirectionIdEnd);
          }
          previousWindDirectionId = windDirectionIdEnd;
        }
    ...
    const WindEventId& eventId) const
{
  ...
  bool useTimePhrase = true;  //! fit_into_large_day_part(speedEventPeriod);
                                ...
                                theParameters.theWindDataVector))
  {
    if (useTimePhrase && !firstSentenceInTheStory)
    {
      sentence << Delimiter(COMMA_PUNCTUATION_MARK)
               << getTimePhrase(periodEndPhrasePeriod,
  ...
  if (speedChangePeriods.size() <= 1)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        get_time_phrase_large(
            periodEnd, false, theParameters.theVar, dayPhasePhrasePeriodEnd, false);
        ...
        else
        {
          ...
          logWindSpeedPeriod(periodEnd,
                             windSpeedIntervalSentence(
                                 periodEnd, USE_AT_ITS_STRONGEST_PHRASE && !windIsDecreasing));
        }
      }
    }
  }
  else
  {
    ...
    for (unsigned int i = 1; i < speedChangePeriods.size(); i++)
    {
      ...
      if (!lastPeriod)
      {
        ...
        if (abs(lowerLimit2 - lowerLimit1) <= 1.0 && abs(upperLimit2 - upperLimit1) <= 1.0)
          continue;
      }

      Sentence intervalSentence;
      intervalSentence << windSpeedIntervalSentence(
          lastPeriod ? speedChangePeriods[i] : currentPeriodEnd,
          lastPeriod && USE_AT_ITS_STRONGEST_PHRASE && !windIsDecreasing);
      ...
      if (!intervalSentence.empty())
      {
        if (useTimePhrase)
        {
          ...
          TextGenPosixTime phrasePeriodEndTimestamp(lastPeriod
                                                        ? speedChangePeriods[i].localStartTime()
                                                        : currentPeriodEnd.localStartTime());
                                     ...
                                     const bool& useAlkaenPhrase) const
{
  ...
  /*
  if(!specifyDay && thePreviousDayNumber == thePeriod.localStartTime().GetWeekday() &&
  partOfTheDayLarge == YO)
          specifyDay = true;
  */
    ...
    const WindEventId& eventId) const
{
  ...
  WeatherPeriod speedEventPeriod(eventPeriodDataItem.theWindSpeedChangePeriod);
  // find the point of time when max wind has changed theParameters.theWindSpeedThreshold from the
  // previous
...
// tuuli voi voimistua, heiket‰ tai k‰‰nty‰ usealla per‰kk‰isel‰ periodilla (k‰yr‰ polveilee, mutta
// ei k‰‰nny)
                                          ...
                                          bool theUseAtItsStrongestPhrase) const
{
  ...
  float intervalLowerLimit(medianvalue);
  float intervalUpperLimit(maxvalue);
...
Sentence WindForecast::windDirectionSentenceWithAttribute(
    const WindDirectionId& theWindDirectionId) const
{
  Sentence sentence;
...
std::string WindForecast::getWindDirectionTurntoString(
    const WindDirectionId& theWindDirectionId) const
{
  std::string retval;
...
bool WindForecast::windDirectionTurns(const WeatherPeriod& thePeriod) const
{
  ...
  return wind_direction_turns(
      windDirectionBeg, windDirectionEnd, windTopSpeedBeg, windTopSpeedEnd, theParameters.theVar);
}
...
float WindDataItemUnit::getWindSpeedShare(const float& theLowerLimit,
    ...
    const WindDirectionId& windDirectionId)
{
  ...
  for (unsigned int i = 0; i < theWindDirectionDistribution.size(); i++)
  {
    ...
    WindDirectionId directionId = wind_direction_id(directionVar, speedVar, "");
                               ...
                               const WindDirectionId& windDirectionId)
{
  ...
  for (unsigned int i = 0; i < theParameters.theWindDataVector.size(); i++)
  {
    ...
    {
      sumShare += get_wind_direction_share(item.theWindDirectionDistribution, windDirectionId);
      counter++;
    }
  }
    ...
    const string& var)
{
  ...
  for (unsigned int i = 0; i < theWindDataVector.size(); i++)
  {
    ...
    if (item.thePeriod.localStartTime() == pointOfTime)
    {
      ...
      directionId =
          wind_direction_id(item.theEqualizedWindDirection, item.theEqualizedTopWind, var);
    }
  }
    ...
    const string& var)
{
  ...
  WeatherResult resultDirection =
      mean_wind_direction(theSources, theArea, period, medianWindSpeed, topWindSpeed, var);
...
WeatherResult get_wind_direction_at(const wind_data_item_vector& theWindDataVector,
                                    ...
                                    const string& var)
{
  return (get_wind_direction_pair_at(theWindDataVector, theArea, pointOfTime, var).first);
}
...
WeatherResult get_wind_direction_at(const wind_data_item_vector& theWindDataVector,
                                    ...
                                    const string& var)
{
  return (get_wind_direction_pair_at(theWindDataVector, theArea, theSources, period, var).first);
}
...
WindDirectionId get_wind_direction_id_at(const wind_data_item_vector& theWindDataVector,
                                         ...
                                         const string& var)
{
  return (get_wind_direction_pair_at(theWindDataVector, theArea, pointOfTime, var).second);
}
...
WindDirectionId get_wind_direction_id_at(const wind_data_item_vector& theWindDataVector,
                                         ...
                                         const string& var)
{
  return (get_wind_direction_pair_at(theWindDataVector, theArea, theSources, period, var).second);
}
                              ...
                              const wind_data_item_vector& windDataVector)
{
  ...
  get_wind_speed_interval(
      thePeriod.localStartTime(), theArea, windDataVector, begLowerLimit, begUpperLimit);
  get_wind_speed_interval(
      thePeriod.localEndTime(), theArea, windDataVector, endLowerLimit, endUpperLimit);

  // 80% of upper limit change and 20% of lower limit change
  int begSpeed = static_cast<int>(round(begUpperLimit * 0.8 + begLowerLimit * 0.2));
  int endSpeed = static_cast<int>(round(endUpperLimit * 0.8 + endLowerLimit * 0.2));

  int difference = endSpeed - begSpeed;

  if (abs(difference) <= round(theWindSpeedThreshold))
    return false;
  else
    return true;
}
                          ...
                          const wind_data_item_vector& windDataVector)
{
  ...
  get_wind_speed_interval(thePeriod1, theArea, windDataVector, lowerLim1, upperLim1);

  get_wind_speed_interval(thePeriod2, theArea, windDataVector, lowerLim2, upperLim2);
...
{
  float begLower(begLowerLimit), begUpper(begUpperLimit), endLower(endLowerLimit),
      endUpper(endUpperLimit);

  get_wind_speed_interval(thePeriod.localStartTime(), theArea, windDataVector, begLower, begUpper);
  get_wind_speed_interval(thePeriod.localEndTime(), theArea, windDataVector, endLower, endUpper);

  begLowerLimit = round(begLower);
  begUpperLimit = round(begUpper);
  endLowerLimit = round(endLower);
  endUpperLimit = round(endUpper);
...
void get_wind_speed_interval(const WeatherPeriod& thePeriod,
                             ...
                             float& upperLimit)
{
  get_plain_wind_speed_interval(thePeriod, theArea, windDataVector, lowerLimit, upperLimit);

  windspeed_distribution_interval(thePeriod, theArea, windDataVector, lowerLimit, upperLimit);
}
...
void get_wind_speed_interval(const TextGenPosixTime& pointOfTime,
                             ...
                             float& upperLimit)
{
  get_plain_wind_speed_interval(pointOfTime, theArea, windDataVector, lowerLimit, upperLimit);

  WeatherPeriod period(pointOfTime, pointOfTime);

  windspeed_distribution_interval(period, theArea, windDataVector, lowerLimit, upperLimit);
}
/*
void get_wind_speed_interval(const WeatherPeriod& thePeriod,
                           const WeatherArea& theArea,
                                ...
                                WeatherResult& windDirectionInland)
{
  ...
  windDirectionFull =
      get_wind_direction_at(theWindDataVector, theFullArea, theSources, thePeriod, theVar);

  windDirectionCoastal =
      get_wind_direction_at(theWindDataVector, coastalArea, theSources, thePeriod, theVar);

  windDirectionInland =
      get_wind_direction_at(theWindDataVector, inlandArea, theSources, thePeriod, theVar);
}
