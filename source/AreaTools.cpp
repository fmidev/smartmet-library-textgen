...
namespace TextGen
{
  namespace AreaTools
  {
	...
	Rect::Rect(const double& topLeftX, const double& topLeftY, const double& bottomRightX, const double& bottomRightY)
	{
	...
	Rect Rect::boundingRect(const Rect& theRect) const
	{
	  ...
	  double maxX = theRect.m_bottomRight.X() > m_bottomRight.X() ? theRect.m_bottomRight.X() : m_bottomRight.X();
	  double minY = theRect.m_bottomRight.Y() < m_bottomRight.Y() ? theRect.m_bottomRight.Y() : m_bottomRight.Y();
	...
	bool Rect::contains(const Rect& theRect) const
	{
	  ...
	  return(m_topLeft.X() <= theRect.m_topLeft.X() &&
			 ...
			 m_bottomRight.Y() <= theRect.m_bottomRight.Y());
	}

	bool Rect::contains(const NFmiPoint& thePoint) const
	{
	  return(m_topLeft.X() <= thePoint.X() &&
			 m_bottomRight.X() >=  thePoint.X() &&
			 m_topLeft.Y() >= thePoint.Y() &&
			 m_bottomRight.Y() <= thePoint.Y());
	}
