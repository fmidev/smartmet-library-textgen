...
- New "huomenna <part of day>" dictionary keys in all languages; translations outside fi, sv and en await native review
- Documented that string settings are written without quotes (tolerated from smartmet-library-calculator 26.9.21)
...
- Added the new "voimakkaita puuskia" dictionary key (introduced with the tiered convective-cell phrasing) to every .po file under textgen/po/ with a per-language translation derived from each file's existing "hyvin voimakkaita puuskia" rendering (only sonera.po is intentionally untouched — it does not carry the cell phrases)
- Convective cell sentence now includes a time-of-day qualifier (e.g. "illalla") when the period maps cleanly to a part of day via get_narrow_time_phrase. Phrasing built from three new template dictionary keys (plain / one-qualifier / two-qualifier) with positional [N] placeholders so each language's msgstr can reorder time-vs-quadrant freely. Quadrant phrases (pohjoisosissa, etelaosissa, itaosissa, lansiosissa) are now first-class dictionary keys instead of raw Finnish strings — added with translations to all 22 multilingual .po files
- Collapse multiple convective cells into a single follow-up sentence: report the strongest peak gust over the union period (with shared dominant quadrant if every cell agrees). Per-cell sentences read as noise when several adjacent cells fired; the diagnostic log still lists each cell individually. When the union spans three or more parts of day (which get_narrow_time_phrase doesn't have a combined label for), the time falls back to the strongest cell's own period
- Renormalize the per-timestep wind speed distributions after a convective cell is removed, so the post-removal distribution still sums to its original total instead of leaving a deficit. Bare zeroing of the high-wind buckets could silently break downstream coverage walks: WindForecast::get_peak_wind seeks a 98% cumulative-share threshold, and after a cell removal of (say) 15% the cumulative might never cross 98%, leaving peak/peak-time fields at their default and producing a bogus "peak: 0" downstream that propagated into a "4...6" minimum-interval-size fallback in the strengthening sentence ("in the evening 4-6 m/s" after a previous "4-8 m/s" period — reading as decreasing while the wind was actually strengthening)
- Insert the convective cell sentence chronologically between wind sentences instead of appending at the end, so the narrative reads in time order. WindForecast now exposes getWindStoryParts() returning per-sentence-info Paragraphs tagged with their WeatherPeriod; wind_overview anchors the cell sentence after the wind sentence whose period contains the strongest cell's start. When inserted between wind sentences the cell sentence drops its own time qualifier — the surrounding wind text already pins the time, and repeating "in the evening" would read poorly
- Reset theContextualMaxIntervalSize to theMaxIntervalSize at the start of every get_plain_wind_speed_interval call. The cap (lowered to 4 m/s when a period's top wind is below the warning threshold) is supposed to be per-period but was mutating shared story params, so an earlier call evaluating a low-wind sub-period (e.g. the start of a long strengthening at 9.9 m/s) stuck the cap at 4 for the rest of the run. That narrowed the strengthening's report at its tail (top wind 14.6 m/s, well above threshold) from a sensible 7..15 down to 7..11, producing a sentence that read as decreasing despite "increasing further"
- po/en.po: fixed the long-standing "strenghten*" typo to "strengthen*" everywhere (38 lines), fixed "contines" typo, and corrected the grammar of the "continues to [2]" template at line 5321 to "continues [2]" — the [2] placeholder substitutes a present participle ("strengthening" / "weakening"), so "continues to strengthening" was ungrammatical. The other "continues to strengthen" literals in en.po use the infinitive directly and remain correct after the spelling fix.
- Added min_weakening_duration setting (default 2 hours): suppress wind-weakening events shorter than this duration. Short standalone weakening events are usually spurious — a brief dip in the equalized base wind that survives convective cell removal — and reading them as standalone "wind weakens" sentences misleads the reader. Setting the knob to 0 disables the suppression.
- Tautology resolver: when shifting a sentence's period to find a different part-of-day time phrase, require the shifted period to still overlap with the original event period. Without the overlap constraint a 1-hour weakening at ilta could have its time phrase nudged forward two hours into iltayö (entirely outside the actual event), producing sentences whose time phrase points to a slot in which nothing actually happens. With the constraint, when no overlapping shift yields a different part-of-day, the resolver keeps the original (tautological) time phrase rather than fabricating a misleading alternative.
...
* Thu May 15 2025 Mika Heiskanen <mika.heiskanen@fmi.fi> - 25.5.15-1.fmi
- Clarified code by naming some magic constants
