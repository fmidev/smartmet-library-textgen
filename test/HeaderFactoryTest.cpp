...
namespace HeaderFactoryTest
{
  ...
  void header_from_until()
  {
	...
	result = require("sv",area,period1,var,"Utsikter från och med söndag morgon till måndag morgon");
	if(!result.empty()) TEST_FAILED(result.c_str());
	...
	result = require("sv",area,period2,var,"Utsikter från och med söndag morgon till måndag kväll");
	if(!result.empty()) TEST_FAILED(result.c_str());
  ...
  void header_several_days()
  {
	...
	result = require("sv",area,period,var,"Från och med söndag morgon för de följande två dygnen");
	if(!result.empty()) TEST_FAILED(result.c_str());
