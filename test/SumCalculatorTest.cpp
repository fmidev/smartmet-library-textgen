...
void sum()
{
  ...
  calc(2);
  if (calc() != 1 + 2) TEST_FAILED("Failed to return 1+2");

  calc(3);
  if (calc() != 1 + 2 + 3) TEST_FAILED("Failed to return 1+2+3");

  calc(4);
  if (calc() != 1 + 2 + 3 + 4) TEST_FAILED("Failed to return 1+2+3+4");
...
int main(void)
{
  ...
  cout << endl << "SumCalculator tests" << endl << "===================" << endl;
