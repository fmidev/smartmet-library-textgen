...
void mask()
{
  ...
  typedef CoastMaskSource::mask_type mask_type;
  ...
  CoastMaskSource source(rannikko);
  ...
  // not even near the coast
  {
    ...
    if (m->size() != 0)
      TEST_FAILED("Size of mask pohjois_lappi should be 0, not " + Convert(m->size()));
  }
...
int main(void)
{
  cout << endl << "CoastMaskSource tester" << endl << "======================" << endl;
  CoastMaskSourceTest::tests t;
  return t.run();
}
