...
void settings_require()
{
  ...
  if (posixTime != TextGenPosixTime(2013, 5, 6, 13, 58))
    TEST_FAILED("Value of time_param sould be '201305061358' not " +
                posixTime.ToStr(kYYYYMMDDHHMM));

  // result
  TextGen::WeatherResult result(Settings::require_result("result_param"));
  if (result != TextGen::WeatherResult(12.50, 30.20))
    TEST_FAILED("Value of result_param sould be '12.50,30.20' not " +
                posixTime.ToStr(kYYYYMMDDHHMM));
...
int main(void)
{
  ...
  cout << endl << "Settings tests" << endl << "====================" << endl;
